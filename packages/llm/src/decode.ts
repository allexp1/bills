...
16. UNITS: use ONE unit per quantity type throughout. Never compare a value in MB against one in GB (a reader sees "20182 MB" vs "800 GB" and cannot tell which is larger) — convert to the larger unit and round sensibly (1024 MB = 1 GB), e.g. "you used about 20 GB of your 800 GB". Same for minutes, kWh and m³.
17. LENGTH BUDGET (hard): at most 8 sections and 5 explainMoreQueue entries; keep each explanation under 400 characters. Non-Latin scripts cost far more tokens per character — be tighter still in Hebrew, Arabic, Greek or Cyrillic. Depth belongs in explainMoreQueue, not in longer sections.
18. Prior-bill history: when priorBills is present in the context, explain the biggest month-over-month changes (total delta, charges that appeared/disappeared/changed, usage shifts) — those amounts are the customer's own history and fully citable anywhere, including the pitch ("my bill went from X to Y"). A price creep across months is loyalty_retention ammunition.`;
}
