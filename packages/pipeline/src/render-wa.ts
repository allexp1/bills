...
const STRINGS: Record<SupportedLocale, Record<string, string>> = {
  en: {
    ...
    duplicateBill: "You have already sent me this bill 🙂 Here is the analysis from {when}: {link}",
  ...
  es: {
    ...
    duplicateBill: "Ya me habías enviado esta factura 🙂 Aquí tienes el análisis del {when}: {link}",
  ...
  fr: {
    ...
    duplicateBill: "Vous m’aviez déjà envoyé cette facture 🙂 Voici l’analyse du {when} : {link}",
  ...
  pt: {
    ...
    duplicateBill: "Já me tinha enviado esta fatura 🙂 Aqui está a análise de {when}: {link}",
  ...
  de: {
    ...
    duplicateBill: "Diese Rechnung haben Sie mir schon geschickt 🙂 Hier ist die Analyse vom {when}: {link}",
  ...
  he: {
    ...
    duplicateBill: "כבר שלחת לי את החשבון הזה 🙂 הנה הניתוח מהתאריך {when}: {link}",
  ...
  ru: {
    ...
    duplicateBill: "Вы уже присылали мне этот счёт 🙂 Вот разбор от {when}: {link}",
  ...
  zh: {
    ...
    duplicateBill: "这份账单您已经发送过了 🙂 这是 {when} 的分析：{link}",
